<template>
  <a-modal v-model="show" title="修改设备上下线" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='设备上下线名称' v-bind="formItemLayout">
            <a-input v-decorator="[
            'name',
        ...
        <a-col :span="12">
          <a-form-item label='设备上下线类型' v-bind="formItemLayout">
...
export default {
  ...
  computed: {
    ...
    show: {
      get: function () {
        return this.offlineEditVisiable
      },
      set: function () {
      }
    }
  ...
  methods: {
    selectTypeList () {
      this.$get('/cos/offline-type/list').then((r) => {
    ...
    setFormValues ({...offline}) {
      this.rowId = offline.id
      let fields = ['name', 'model', 'typeId', 'unit', 'purchasePrice', 'sellPrice', 'remark']
      let obj = {}
      Object.keys(offline).forEach((key) => {
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(offline['images'])
        }
